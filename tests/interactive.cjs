/* Проверяем, что ссылки ведут на существующие страницы, а кнопки работают.
   Только гостевой режим: не входит в аккаунт и не пишет в облако. */
...
/* Compare actual visible DOM content, not innerHTML.length: two different messages
   can have the same length, falsely marking a working reset button as dead. */
const snapshot=()=>({
...
(async()=>{
 ...
 try{
  for(const path of PAGES){
   ...
   await page.addInitScript(GUEST);await page.goto(BASE+path,{waitUntil:'domcontentloaded'});await page.waitForTimeout(900);
   const links=await page.evaluate(()=>[...document.querySelectorAll('a[href]')].map(node=>({
    href:node.getAttribute('href'),resolved:node.href,text:(node.textContent||'').trim().slice(0,30),
   ...
   for(const item of links){
    ...
    if(item.href.startsWith('http')&&!item.resolved.startsWith(BASE))continue;
    if(item.href.startsWith('mailto:')||item.href.startsWith('#'))continue;
    const target=new URL(item.resolved),response=await page.request.get(target.origin+target.pathname);
   ...
   const buttons=await page.evaluate(()=>[...document.querySelectorAll('button')].map((node,index)=>({
    index,id:node.id,text:(node.textContent||'').trim().slice(0,34),
    visible:node.getBoundingClientRect().width>0&&node.getBoundingClientRect().height>0,
    disabled:node.disabled,inDialog:Boolean(node.closest('dialog'))
   })));
   for(const info of buttons){
    ...
    const now=await button.evaluate(node=>({disabled:node.disabled,pressed:node.getAttribute('aria-pressed'),visible:node.getBoundingClientRect().width>0&&node.getBoundingClientRect().height>0})).catch(()=>null);
    if(!now||now.disabled||!now.visible||now.pressed==='true'||SPEECH.test(info.text))continue;
    const invalidForm=await button.evaluate(node=>{const form=node.form||node.closest('form');return Boolean(form&&node.type==='submit'&&!form.checkValidity());}).catch(()=>false);
    ...
    if(JSON.stringify(before)===JSON.stringify(after)&&!IDEMPOTENT.has(info.id))suspects.push({path,text:info.text});
   ...
   await context.close();console.log(`проверено ${path}: ${links.length} ссылок, ${buttons.filter(item=>item.visible&&!item.inDialog).length} кнопок`);
  }
  for(const suspect of suspects){
   const context=await browser.newContext({viewport:{width:1280,height:900}}),page=await context.newPage();
   await page.addInitScript(GUEST);await page.goto(BASE+suspect.path,{waitUntil:'domcontentloaded'});await page.waitForTimeout(900);
   ...
   const before=await page.evaluate(snapshot);
   await button.click({timeout:4000}).catch(()=>{});await page.waitForTimeout(300);
   const after=await page.evaluate(snapshot);
   if(JSON.stringify(before)===JSON.stringify(after))problems.push(`${suspect.path}: кнопка «${suspect.text}» ничего не меняет и на чистой странице`);
   await context.close();
  }
