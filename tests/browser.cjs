/* Public, guest-only browser check. Never logs in or modifies real user data. */
...
(async()=>{
  ...
  try{
    for(const config of [{name:'desktop',width:1440,height:900},{name:'mobile',width:390,height:844},{name:'small-mobile',width:320,height:720}]){
      ...
      assert.equal(await page.locator('#qualityPanel').count(),1);
      ...
      for(let index=0;index<4;index++){
        ...
        await page.locator('.quiz-choices button').filter({hasText:answer}).click();
      }
      ...
      console.log(`PASS: ${config.name} lessons, revision, feedback and responsive layout`);
      await page.close();
    }
    ...
    await page.waitForFunction(()=>document.querySelector('#output').textContent.includes('Python загружен.')||document.querySelector('#output').textContent.includes('Не удалось загрузить Python'),{timeout:90000});
    assert.match(await page.locator('#output').textContent(),/Python загружен/,'Python worker needs to load.');
    await page.locator('#run').click();
    await page.waitForFunction(()=>document.querySelector('#output').textContent.trim()==='3',{timeout:25000});
