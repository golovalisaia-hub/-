...
(async()=>{
 ...
 try{
  for(const width of [1440,390,320]){
   ...
   assert.equal(await page.locator('#save').isDisabled(),true,'Guest cannot write');
  ...
  console.log('PASS: independent QA and English evidence, verified draft reload, self-review gate, next lesson and zero planner writes.');
  await page.close();
