/* New QA-first path. No writes to SEVER tasks and no inference from legacy calendar checkmarks. */
(()=>{'use strict';
...
function choiceButton(text,question,optionIndex,correctIndex,quizId){
  ...
  if(passed.get(key(lesson,subject))?.has(quizId)){
    ...
    if(optionIndex===correctIndex){
      ...
      button.classList.add('incorrect');feedback('Пока неверно. Вернись к объяснению и попробуй другой вариант.','bad');
    }
  });
  return button;
}
function renderQuiz(){
  ...
  for(let i=0;i<questions.length;i++){
    ...
    options.forEach((name,index)=>choices.append(choiceButton(name,questions[i],index,answer,i)));
...
function render(){
  ...
  if(cloud?.status==='practiced'&&!passed.has(key(lesson,subject)))passed.set(key(lesson,subject),new Set([0,1]));
  $('save').disabled=!ready||saving;$('complete').disabled=!ready||saving;
  $('previous').disabled=lesson===1;$('next').disabled=lesson===total;
...
function navigate(n,s=subject){if(saving||n<1||n>total||!SUBJECTS[s])return;lesson=n;subject=s;render();}
async function write(desired){
  ...
  saving=true;$('save').disabled=true;$('complete').disabled=true;feedback('Сохраняем в облаке…');
  ...
  try{
    ...
    if(result.error||result.data?.user_id!==user.id||result.data?.track!==payload.track||result.data?.lesson_number!==n||result.data?.status!==desired)throw result.error||Error('Server did not confirm expected row.');
    progress.set(key(n,s),result.data);localClear(n,s);
  ...
  finally{saving=false;$('save').disabled=!ready;$('complete').disabled=!ready;}
}
async function loadCloud(){
  ready=false;$('save').disabled=true;$('complete').disabled=true;
  ...
  progress.clear();for(const row of result.data||[])progress.set(`${row.track}:${row.lesson_number}`,row);
  ready=true;
...
async function authorize(){
  ...
  try{
    ...
    if(!result.data.user){user=null;ready=false;status('Гостевой просмотр: для сохранения войди в тот же аккаунт, что используешь в SEVER.');render();return;}
